...
def fas_invert(runname=cfg.RUNNAME, sptdbpath=cfg.SPTDBPATH, 
               outpath=cfg.RUNPATH, hcomponent='R', 
               model_name='malagniniQ0', method='SLSQP', jac='exact',
               use_uncert='noeps', ref_ampl='meanA', subtract_noise=False,
    ...
    run_name = utils.set_run_label(model_name, use_uncert, weights, bounds,
                                   ...
                                   hcomponent)
    runpath = cfg.RUNPATH + '/' + run_name 
    if os.path.exists(runpath) == 0:
        os.makedirs(runpath)
    ...
    if jac == 'exact':
        ...
        if 1: 
            ...
            utils.writelog(runpath, filename, fmin, pars_dict,
                           p_input, p_model, p_out)
        ...
        print('------------------')
        if 1:
            filename = run_name + '_nojac.txt'
            utils.writelog(runpath, filename, fmin, pars_dict, 
                           p_input, p_model, p_out)

    if save is True:
        with open(runpath + '/' + run_name + '.pkl', 'wb') as fout:
